...
def get_standard_shader_program() -> pyglet.graphics.shader.ShaderProgram:
...
def get_test_rectangle(x, y, width, height, program: pyglet.graphics.shader.ShaderProgram, batch=None, group=None):
    ...
    return program.vertex_list_indexed(6, pyglet.gl.GL_TRIANGLES,
...
class App(pyglet.window.Window):
    def __init__(self):
        ...
        self.program = get_standard_shader_program()
        ...
        # scene elements
        n = 30
        self.rectangles = [get_test_rectangle(self.width/2 + i*50, self.height/2 + j*50, 30, 30, self.program, self.batch) for i in range(n) for j in range(n)]

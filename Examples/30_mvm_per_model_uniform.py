...
class App(pyglet.window.Window):
    def __init__(self, **kwargs):
        ...
        self.cubes = self.get_cubes(n=20)
    ...
    def get_params(self):
        return [random.randrange(-20, 20, 1) * 0.1 for _ in range(len(self.cubes))]

    def get_axes(self):
        return [random.choice([Vec3(0, 1, 0), Vec3(1, 0, 0), Vec3(0, 0, 1)]) for _ in range(len(self.cubes))]

    def on_draw(self) -> None:
        """1) Per model uniform updates method"""
        self.time += 1 / 60
        self.clear()
        self.program.use()

        # static objects
        self.program['model'] = self.zero_rotation
